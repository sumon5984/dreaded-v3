module.exports = async (context) => {
  ...
  try {
    ...
    client.ev.once('messages.upsert', async ({ messages }) => {
      const userReply = messages.find(msg => msg.key.quoted?.id === messageKey.id);

      if (!userReply) return m.reply("No reply detected. Please try again.");
      ...
      if (downloadChoice === 1) {
        try {
          ...
          await m.reply(`_Downloading audio ${title}_`);
        ...
        await m.reply("Sending doc song.");
